...
import { LogoText, LogoTopWhite } from "../svg/logo-text";
import { ArrowDownHeader, ArrowDownHeaderWhite } from "../svg/arrow-down";
...
const HeaderComponent = () => {
  ...
  return (
    <header>
      <div
        className="fixed w-full top-0 left-0 z-50 text-white capitalize"
        style={{ background: background }}
      >
        <div className="py-[14px] bg-white lg:bg-transparent">
          <div className="relative px-5 max-w-[1440px] mx-auto flex justify-between items-center">
            ...
            >
              {isScroll ? <LogoText /> : <LogoTopWhite />}
            </Link>
                      ...
                      <div className="flex items-center gap-1 cursor-pointer">
                        <div
                          className={clsx(
                            ...
                            "xl:text-[16px]",
                            isScroll ? "text-[#0030C0]" : "text-[#ffffff]"
                          )}
                        >
                          {navItem?.title}
                        </div>
                ...
                return (
                  <Link key={index} href={`/${navItem?.value}`}>
                    <div className="flex items-center gap-1">
                      <div
                        className={clsx(
                          "text-[14px] xl:text-[16px] leading-[45px] text-[#0030C0]",
                          isScroll ? "text-[#0030C0]" : "text-[#ffffff]"
                        )}
                      >
                        {navItem?.title}
